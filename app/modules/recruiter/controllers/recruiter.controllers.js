const reruiterRepo = require('../repositories/reruiter.repository');
const userRepo = require('../../user/repositories/user.repository');
class Reruiter {
  async create(sentUser, params) {
    try {
      params.user = sentUser._id;
      const response = await reruiterRepo.save(params);
  ...
  async getRecruiterPosts(user_id, params) {
    try {
      const filterParams = {
        user: user_id,
        isRead: false,
      ...
      const allPosts = await reruiterRepo.getAll(
        filterParams,
      ...
      return {
        ...
        message: 'Reruiter posts fetched',
      };
    } catch (e) {
      throw e;
    }
